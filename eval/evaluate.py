...
class EvaluateModel:
    ...
    def predict_from_file(self, audio_path: str, batch_audio_chunk: int) -> str or None:
        ...
        return _get_lang_by_id(int(np.argmax(np.bincount(list(map(int, predicted))))))
...
class Metrics:
    ...
    def acc_by_lang(self) -> dict:
        ...
        for i in range(len(self.predicted)):
            no_by_lang[_get_lang_by_id(self.labels[i])] += 1
            if self.predicted[i] == self.labels[i]:
                corr_by_lang[_get_lang_by_id(self.labels[i])] += 1
        return {lang: corr_by_lang[lang] / no_by_lang[lang] for lang in Config.class_dict.keys()}


def _get_lang_by_id(lang_id: int) -> str:
